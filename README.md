...
- **Fleet** — Mock 22-van yard until `amazon-fleet-dna4.json` is filled from Console My vehicles. CSV export of the table that is showing. DVIC compares pre-trip to post-trip damage; the Sep 23 capture has no inspection rows, so that section stays empty.
