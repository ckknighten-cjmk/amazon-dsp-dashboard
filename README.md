...
Live CJMK Ops seeds come from Amazon DSP Console: the **Week 38** scorecard (Sep 13–19, 2026, with Week 37 still on the scorecard switcher), the **2026-09-25** Delivery Execution board, the **Week 38** 10-hour bonus list, and the **Week 38** and **Week 39** schedule workbooks used by Compliance. Fleet and incidents in CJMK Ops are still mock fixtures. Do not invent metrics that Console did not publish.
...
| Surface | Source |
| --- | --- |
| **Routes / Delivery Execution** | Amazon DSP Console Delivery Execution **evening wrap** for **2026-09-25** (~8:13 p.m. CT; 33 routes, 2 in progress, 31 complete, 0 incomplete, associates, stops, packages, board totals) plus the exception Packages CSV (241 rows). Board package-status chips use Console totals (remaining **9**, reattemptable **4**, undeliverable **2**, missing **4**, RTS **19**, pickup failed **212**). Those export statuses match the CSV on this capture. Route-row packages are **9,626** delivered of **9,635** planned. Vehicle and on-time % were **not** on the Console board — the UI leaves them unavailable and does not invent them. Console `rescueActions` is **0**. A route is marked **Rescue: Yes** when it lists more than one associate (15 multi-transporter routes). The first associate is primary and the rest are rescuers. That column is not Amazon’s official rescue flag. Gauges are 99% locations, stops, packages, and attempt success. Other board chips in the seed: unknown stops **25**, work-hour risk **1**, no breaks taken **3**, ahead **1**, at risk **1**. |
...
Station clock is frozen at **Fri Sep 25, 2026 · 8:13 p.m. CT** (the evening-wrap Console capture time).

### Pages

- **Overview** — KPIs from the Sep 25 evening-wrap Delivery Execution board (99% package gauge, remaining 9 / reattemptable 4 / undeliverable 2, 2 in-progress routes). Package delivered counts are the sum of Console route rows (9,626 of 9,635). No prior-day deltas.
...
- **Routes** — Evening-wrap Delivery Execution board: 33 routes (2 in progress, 31 complete, 0 incomplete), gauges at 99% / 99% / 99% / 99% attempt success, package-status chips (remaining 9, reattemptable 4, undeliverable 2, missing 4, RTS 19, pickup failed 212), work-hour risk 1, multi-transporter 15, unknown stops 25, on-road pickups 6/6, the 241-row exception-package export, and a Rescue column for 15 multi-associate routes. CSV downloads sit next to the routes and exceptions tables.
- **Drivers** — Full Weeks 38–39 schedule roster, plus ADP-only names, with Sep 25 route activity merged by name. Census phone and email attach on a unique name match. The page opens on Active. CSV export downloads the full roster, including blank phones.
...
The sticky top bar **Today / This week** control drives Overview KPIs. The week toggle still only has Sep 25 in this Console pull.
...
- [`src/lib/data/seed/delivery-execution-2026-09-25.json`](src/lib/data/seed/delivery-execution-2026-09-25.json) — live Delivery Execution evening wrap
- [`src/lib/data/seed/packages-exceptions-2026-09-25.csv`](src/lib/data/seed/packages-exceptions-2026-09-25.csv) — Packages export for that wrap (241 rows)
