...
Live CJMK Ops seeds come from Amazon DSP Console: the **Week 38** scorecard (Sep 13–19, 2026, with Week 37 still on the scorecard switcher), the **2026-09-23** Delivery Execution board, the **Week 38** 10-hour bonus list, and the **Week 38** and **Week 39** schedule workbooks used by Compliance. Fleet and incidents in CJMK Ops are still mock fixtures. Do not invent metrics that Console did not publish.
...
| Surface | Source |
| --- | --- |
| **Routes / Delivery Execution** | Amazon DSP Console Delivery Execution **evening wrap** for **2026-09-23** (~8:15 p.m. CT; 35 routes, 0 in progress, 33 complete, 2 incomplete, associates, stops, packages, board totals) plus the exception Packages CSV (114 rows). Board package-status chips use Console totals (remaining **2**, reattemptable **2**, undeliverable **7**, missing **8**, RTS **25**, pickup failed **72**). Those export statuses match the CSV on this capture. Route-row packages are **10,941** delivered of **10,943** planned. Vehicle and on-time % were **not** on the Console board — the UI leaves them unavailable and does not invent them. Console `rescueActions` is **0**. A route is marked **Rescue: Yes** when it lists more than one associate (18 multi-transporter routes). The first associate is primary and the rest are rescuers. That column is not Amazon’s official rescue flag. |
...
Station clock is frozen at **Wed Sep 23, 2026 · 8:15 p.m. CT** (the evening-wrap Console capture time).

### Pages

- **Overview** — KPIs from the Sep 23 evening-wrap Delivery Execution board (99% package gauge, remaining 2 / reattemptable 2 / undeliverable 7, 0 in-progress routes). Package delivered counts are the sum of Console route rows (10,941 of 10,943). No prior-day deltas.
...
- **Routes** — Evening-wrap Delivery Execution board: 35 routes, 99% gauges, package-status chips (remaining 2, reattemptable 2, undeliverable 7, missing 8, RTS 25, pickup failed 72), work-hour risk 1, on-road pickups 4/4, the 114-row exception-package export, and a Rescue column for 18 multi-associate routes. CSV downloads sit next to the routes and exceptions tables.
- **Drivers** — Full Weeks 38–39 schedule roster, plus ADP-only names, with Sep 23 route activity merged by name. Census phone and email attach on a unique name match. The page opens on Active. CSV export downloads the full roster, including blank phones.
...
The sticky top bar **Today / This week** control drives Overview KPIs. The week toggle still only has Sep 23 in this Console pull.
...
- [`src/lib/data/seed/delivery-execution-2026-09-23.json`](src/lib/data/seed/delivery-execution-2026-09-23.json) — live Delivery Execution evening wrap
- [`src/lib/data/seed/packages-exceptions-2026-09-23.csv`](src/lib/data/seed/packages-exceptions-2026-09-23.csv) — Packages export for that wrap (114 rows)
