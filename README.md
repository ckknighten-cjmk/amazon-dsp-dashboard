...
| **Drivers** | Full active roster: union of Amazon schedule associates for **Weeks 38 and 39** (transporter ID). Sep 21 Delivery Execution supplies today’s routes, packages, and status when the name matches. ADP Group Timecard names with no schedule transporter ID are included and labeled **ADP only**. Tenure, phone, and scorecard contribution were not in these exports. |
| **Fleet / incidents** | Fleet stays the **22-van mock** until [`src/lib/data/seed/amazon-fleet-dna4.json`](src/lib/data/seed/amazon-fleet-dna4.json) has rows. That file is the drop-in for **Administration → Fleet → My vehicles** (`unit`, `plate`, `makeModel`, `vin`, `ownership`, `type`, `status`, `statusReason`, `lastRouteCompleted`, `assignedRoute`). Week 38/39 schedules and the Sep 21 Delivery Execution board do not name vans. Do not invent VINs or plates. Year and mileage are left empty on Console rows. DVIC for **2026-09-23** is **0** pre-trip, **0** post-trip, **0** AVI post-trip, with no damage rows. Incidents remain mock. |
...
- **Scorecard** — DSP Console Week 37: overall 85.8 Fantastic, plus Safety, Quality, and Service Reliability metrics. CSV export uses the values shown on the page. Prior week stays “Not shown in Console.”
...
- **Drivers** — Full Weeks 38–39 schedule roster, plus ADP-only names, with Sep 21 route activity merged by name. The count is filtered of that full list. CSV export downloads the roster.
- **Fleet** — Mock 22-van yard until `amazon-fleet-dna4.json` is filled from Console My vehicles. CSV export of the table that is showing. DVIC pairs pre-trip and post-trip for the same vehicle and service day and marks post-trip damage the pre-trip did not list. The Sep 23 Fleet Dashboard capture is 0 / 0 / 0 with no checklist rows, so that section stays empty until `src/lib/data/seed/dvic-2026-09-23.json` is filled. CSV export downloads those pairs.
- **Incidents** — Mock DVR, complaint, vehicle, and safety log
- **Payments** — Flex Payments settlements seeded **2026-09-21**: pending action, paid total, invoice table, Week 37 breakdowns, and 2025 YTD Insights. CSV export includes those sections.
