...
Live CJMK Ops seeds come from Amazon DSP Console: the **Week 38** scorecard (Sep 13–19, 2026, with Week 37 still on the scorecard switcher), the **2026-09-24** Delivery Execution board, the **Week 38** 10-hour bonus list, and the **Week 38** and **Week 39** schedule workbooks used by Compliance. Fleet and incidents in CJMK Ops are still mock fixtures. Do not invent metrics that Console did not publish.
...
| Surface | Source |
| --- | --- |
| **Routes / Delivery Execution** | Amazon DSP Console Delivery Execution **evening wrap** for **2026-09-24** (~8:10 p.m. CT; 35 routes, 6 in progress, 29 complete, 0 incomplete, associates, stops, packages, board totals) plus the exception Packages CSV (102 rows). Board package-status chips use Console totals (remaining **15**, reattemptable **0**, undeliverable **6**, missing **9**, RTS **36**, pickup failed **54**). The CSV matches missing and RTS. It counts undeliverable **5** and pickup failed **52**, so those two chips can differ from the table. Remaining is a board chip and is not an export status. Route-row packages are **10,146** delivered of **10,162** planned. Vehicle and on-time % were **not** on the Console board — the UI leaves them unavailable and does not invent them. Console `rescueActions` is **0**. A route is marked **Rescue: Yes** when it lists more than one associate (16 multi-transporter routes). The first associate is primary and the rest are rescuers. That column is not Amazon’s official rescue flag. Gauges are 99% locations, stops, and packages, and 100% attempt success. Other board chips in the seed: unknown stops **27**, inactive **4**, no breaks taken **1**, ahead **4**, at risk **1**. |
...
Station clock is frozen at **Thu Sep 24, 2026 · 8:10 p.m. CT** (the evening-wrap Console capture time).

### Pages

- **Overview** — KPIs from the Sep 24 evening-wrap Delivery Execution board (99% package gauge, remaining 15 / reattemptable 0 / undeliverable 6, 6 in-progress routes). Package delivered counts are the sum of Console route rows (10,146 of 10,162). No prior-day deltas.
...
- **Routes** — Evening-wrap Delivery Execution board: 35 routes (6 in progress, 29 complete, 0 incomplete), gauges at 99% / 99% / 99% / 100% attempt success, package-status chips (remaining 15, reattemptable 0, undeliverable 6, missing 9, RTS 36, pickup failed 54), work-hour risk 0, multi-transporter 16, unknown stops 27, inactive 4, no breaks taken 1, on-road pickups 4/4, the 102-row exception-package export, and a Rescue column for 16 multi-associate routes. CSV downloads sit next to the routes and exceptions tables.
- **Drivers** — Full Weeks 38–39 schedule roster, plus ADP-only names, with Sep 24 route activity merged by name. Census phone and email attach on a unique name match. The page opens on Active. CSV export downloads the full roster, including blank phones.
...
The sticky top bar **Today / This week** control drives Overview KPIs. The week toggle still only has Sep 24 in this Console pull.
...
- [`src/lib/data/seed/delivery-execution-2026-09-24.json`](src/lib/data/seed/delivery-execution-2026-09-24.json) — live Delivery Execution evening wrap
- [`src/lib/data/seed/packages-exceptions-2026-09-24.csv`](src/lib/data/seed/packages-exceptions-2026-09-24.csv) — Packages export for that wrap (102 rows)
- [`src/lib/data/seed/delivery-execution-2026-09-23.json`](src/lib/data/seed/delivery-execution-2026-09-23.json) — earlier Delivery Execution evening wrap, kept in the repo
- [`src/lib/data/seed/packages-exceptions-2026-09-23.csv`](src/lib/data/seed/packages-exceptions-2026-09-23.csv) — Packages export for the Sep 23 wrap (114 rows)
