...
| **Fleet / incidents** | Fleet is still a **22-van mock**. A complete DNA4 list needs the Console export from **Administration → Fleet → My vehicles** (unit, plate, VIN, make/model, ownership, type, status). Do not invent those fields. DVIC for **2026-09-23** is the Fleet Dashboard capture: **0** pre-trip, **0** post-trip, **0** AVI post-trip, and no damage rows. New damage is scored only when inspection rows exist. Incidents remain mock. |
...
- **Fleet** — Mock 22-van yard, CSV export of that table, and a DVIC section that compares pre-trip to post-trip damage. The Sep 23 capture has no inspection rows, so the section stays empty.
