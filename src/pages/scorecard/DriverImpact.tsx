...
import { formatContribution } from "../../scorecard/ui";
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="card p-5">
          ...
          <div className="mt-4 space-y-3">
            {leaders.map((driver, index) => (
              <div
                key={driver.driverId}
                className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-3 dark:bg-white/5"
              >
                <div>
                ...
                <div className="text-right">
                  <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">
                    {driver.compositeScore.toFixed(0)}
                  </p>
        ...
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-slate-900 dark:text-white">At-risk drivers</h3>
          <p className="text-xs text-slate-500">Poor metrics, Fair safety, or composite below Great</p>
          <div className="mt-4 space-y-3">
            {risks.map((driver) => (
              <div key={driver.driverId} className="rounded-lg bg-slate-50 px-3 py-3 dark:bg-white/5">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium text-slate-900 dark:text-white">{driver.driverName}</p>
                  <MetricStatusBadge standing={driver.standing} />
                </div>
                <p className="mt-1 text-xs text-rose-600 dark:text-rose-300/80">{driver.riskReasons.join(" · ")}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="card mt-4 overflow-x-auto">
        <div className="border-b border-slate-200 px-5 py-4 dark:border-white/5">
          <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Driver scorecard contributions</h3>
        ...
        <table className="w-full min-w-[72rem] text-left text-sm">
          ...
          <tbody>
            {drivers.map((driver) => (
              <tr key={driver.driverId} className="border-t border-slate-200 dark:border-white/5">
                <td className="px-5 py-3">
                  <p className="font-medium text-slate-900 dark:text-white">{driver.driverName}</p>
                  <p className="text-xs text-slate-500">{driver.driverId}</p>
                </td>
                <td className="px-3 py-3 text-slate-600 dark:text-slate-300">{driver.primaryRoute}</td>
