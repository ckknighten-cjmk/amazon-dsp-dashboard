          ...
          <article key={rec.id} className="card p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h3 className="mt-1 text-lg font-semibold text-white">{rec.title}</h3>
              </div>
            ...
            <p className="mt-3 text-sm leading-6 text-slate-300">{rec.rationale}</p>
            ...
            <div className="mt-4 flex flex-wrap gap-4 border-t border-white/5 pt-4 text-xs text-slate-400">
              ...
              <span className="inline-flex items-center gap-1">
                ...
                {rec.affectedRoutes.length ? rec.affectedRoutes.join(", ") : METRIC_CATALOG[rec.focus === "safety" ? "safety" : rec.focus].label}
              </span>
            </div>
          </article>
        ))}
